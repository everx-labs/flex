...
public:
  ...
  struct error_code : tvm::error_code {
    ...
    static constexpr unsigned null_address                      = 113; ///< Address mustn't be empty
    static constexpr unsigned funds_mismatched                  = 114; ///< The funds does not correspond to the stated
  };
  ...
  void onEverTransfer(
    ...
    ITONTokenWalletPtr dest_handle(dest);
    dest_handle.deploy(wallet_init, 0_ev, SEND_ALL_GAS).acceptMint(tokens, int_sender(), args.evers);
