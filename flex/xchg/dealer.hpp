...
public:
  ...
  process_result process(unsigned sell_idx, unsigned buy_idx) {
    ...
    state.finalize(notify_addr_, sells_.all_amount_, buys_.all_amount_); // finalize state and send AMM notifications
  ...
  deal_result make_deal(OrderInfoXchg sell, OrderInfoXchg buy) {
    ...
    auto sell_extra_return = last_tip3_sell ? (sell.lend_amount - deal_amount) : 0u128;
    auto buy_extra_return = last_tip3_buy ? (buy.lend_amount - deal_amount) : 0u128;
