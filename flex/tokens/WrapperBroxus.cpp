/** \file
 *  \brief WrapperBroxus contract implementation
...
class WrapperBroxus final : public smart_interface<IWrapperBroxus>, public DWrapperBroxus {
public:
  DEFAULT_SUPPORT_FUNCTIONS(IWrapperBroxus, void)

  using data = DWrapperBroxus;
  ...
  struct error_code : tvm::error_code {
    ...
    static constexpr unsigned out_pubkey_must_be_zero           = 111; ///< Out pubkey must be zero (for Broxus tip3 wallet)
    static constexpr unsigned out_owner_must_be_provided        = 112; ///< Out owner must be provided (for Broxus tip3 wallet)
  };
  ...
  void onAcceptTokensTransfer(
    ...
    ITONTokenWalletPtr dest_handle(dest);
    dest_handle.deploy(wallet_init, Evers(args.evers.get())).acceptMint(amount, int_sender(), args.keep_evers, payload);
    total_granted_ += amount;
    tvm_transfer(remainingGasTo, 0, true, SEND_ALL_GAS | IGNORE_ACTION_ERRORS);
  }
  ...
  void burn(
    ...
    require(!out_pubkey, error_code::out_pubkey_must_be_zero);
    require(!!out_owner, error_code::out_owner_must_be_provided);
    ...
    broxus::ITokenWalletPtr(*wallet_)(_remaining_ev()).
      transfer(tokens, *out_owner, out_deploy_value_, answer_addr, notify.has_value(), notify ? *notify : builder().endc());
  ...
  wrapper_details_info getDetails() {
    ...
    return { name_, symbol_, decimals_,
             ...
             *wallet_, getReserveWallet(), *super_root_, cloned_, 2u8 };
  }
  ...
  __always_inline static int _on_bounced(cell /*msg*/, slice msg_body) {
    ...
    auto [hdr, persist] = load_persistent_data<IWrapperBroxus, void, DWrapperBroxus>();
    require(bounced_val <= persist.total_granted_, error_code::wrong_bounced_args);
    persist.total_granted_ -= bounced_val;
    save_persistent_data<IWrapperBroxus, void>(hdr, persist);
    return 0;
  }
...
DEFINE_JSON_ABI(IWrapperBroxus, DWrapperBroxus, EWrapperBroxus);

// ----------------------------- Main entry functions ---------------------- //
MAIN_ENTRY_FUNCTIONS_NO_REPLAY(WrapperBroxus, IWrapperBroxus, DWrapperBroxus)
